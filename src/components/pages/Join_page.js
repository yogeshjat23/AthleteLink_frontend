...
const AboutPage = () => {
  ...
  const handleAddPlayer = async (playerData) => {
    ...
    try { 
      ...
      if (remainingPlayers > 0) {
        const response = await axios.patch(`http://localhost:5000/api/post/${post._id}/add-player`, playerData);
        alert(response.data.message);
       
           const newReamning = remainingPlayers-1 ;

        setTeamDetails(response.data.post.teamDetails); 
        setRemainingPlayers(newReamning); 
  ...
  {teamDetails.map((member, idx) => (
    <div key={idx} className="player">
      ...
      <div className="img-con"> 
        <img src={member.gender === 'Female' ? userfemale : usermale}alt={post.name} className="img" />
